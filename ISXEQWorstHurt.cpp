...
#include "ISXEQWorstHurt.h"

// The mandatory pre-setup function.  Our name is "ISXEQWorstHurt", and the class is ISXEQWorstHurt.
...
// our instanced class.
ISXPreSetup("ISXEQWorstHurt", ISXEQWorstHurt);
...
unsigned int ISXEQWorstHurtXML = 0;
...
bool ISXEQWorstHurt::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQWorstHurt Loaded");
	return true;
}

// shutdown sequence
void ISXEQWorstHurt::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQWorstHurtXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQWorstHurtXML);
...
void ISXEQWorstHurt::ConnectServices()
{
...
void ISXEQWorstHurt::RegisterCommands()
{
	// add any commands
...
void ISXEQWorstHurt::RegisterAliases()
{
	// add any aliases
}

void ISXEQWorstHurt::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQWorstHurt::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQWorstHurt::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQWorstHurt::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQWorstHurt::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQWorstHurt::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQWorstHurtService)
	//        pISInterface->ShutdownService(this,hISXEQWorstHurtService);
}

void ISXEQWorstHurt::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQWorstHurtXML
	ISXEQWorstHurtXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQWorstHurtXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQWorstHurtXML, "General"))
	{
